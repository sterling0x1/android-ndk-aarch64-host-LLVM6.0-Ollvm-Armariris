...
def check_ndk():
    """Verify that our NDK installation is sane"""
    checks = [
        'build/core',
        'prebuilt',
        'platforms',
...
def get_host_tag():
    """Return the host tag for this platform. if not supported."""
    if platform.system() == 'Linux':
        return "linux-arm64" #'linux-x86_64'
...
def get_sysroot_path(arch, api_level):
    """Return the sysroot path for our architecture and API level."""
...
def get_gcc_path(arch, host_tag):
    """Return the GCC path for our host and architecture."""
    toolchain = {
        'arm': 'arm-linux-androideabi',
        'arm64': 'aarch64-linux-android',
        'x86': 'x86',
...
def get_clang_path(host_tag):
    """Return the Clang path for our host."""
...
def make_clang_scripts(install_dir, triple, api, windows):
    ...
    version_number = "7.0.0" #major + minor
...
def main():
    ...
    sysroot_path = get_sysroot_path(args.arch, api)
    gcc_path = get_gcc_path(args.arch, host_tag)
    clang_path = get_clang_path(host_tag)
